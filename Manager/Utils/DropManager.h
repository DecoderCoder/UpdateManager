...
//typedef void (*OnDropCallback)(std::vector<std::wstring> files);

// create a class inheriting from IDropTarget
class DropManager : public IDropTarget
{
public:
	std::function<void(std::vector<std::wstring> files)> dropCallback = nullptr;
	...
	HRESULT Drop(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
	{
		...
		if (SUCCEEDED(pDataObj->GetData(&fmte, &stgm)))
		{
			...
			for (UINT i = 0; i < file_count; i++)
			{
				...
				{
					files.push_back(std::wstring(szFile));
				}
			}
			this->dropCallback(files);
